...
function ListaServicos() {
  ...
  const columns = [
    ...
    { field: 'descricao', header: 'Descricao do serviço' },
  ...
  const onSubmit = (formContent) => {
    ...
    if (formContent.descricao.trim()) {
      let _registros = [...registros];
      let _registro = { ...formContent };
      if (_registro.id) {
        axiosApi.patch("/servicos", formContent)
          .then((response) => {
        ...
        axiosApi.post("/servicos", formContent)
          .then((response) => {
  ...
  const deleteregistro = () => {
    axiosApi.delete("/servicos/" + registro.id)
      .then((response) => {
  ...
  return (
    <>
      ...
      <Sidebar className='w-sidebar-right' header={<h3>Cadastro de serviços</h3>} visible={visibleRight} position="right" blockScroll onHide={() => setVisibleRight(false)} style={{ width: '30em' }}>
